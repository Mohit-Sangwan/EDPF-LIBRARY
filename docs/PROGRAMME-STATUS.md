...
| Automated tests | 1412 executions passing without Docker (1368 tests, plus the 11-test runtime suite on each of 4 frameworks); 24 Tier A parity tests require a daemon |
| Platforms added post-wave | 6 — storage, communication, workflow, AI, MySQL provider, Worker Service host |
| Target frameworks | 4, all four **executing** tests (11/11 identical on each), zero build warnings. net6.0 retired 2026-08-04 ([ADR-038](adr/ADR-038-retire-tier-2.md)) |
...
| **The check-licenses gate failed on every natural input.** It never skipped a CSV header, so the header row was evaluated as a dependency named `Package` whose licence was `License` — unclassified, therefore a violation, therefore a red build. Its sibling `classify-schema` does skip a header, so the two commands had opposite CSV conventions and only one was documented | Running the three CLI commands I had never run — I had only ever run `doctor` | As a build gate that cries wolf on its first use, and gets removed rather than debugged |
| The licence policy listed only current SPDX identifiers, so the deprecated forms real NuGet packages actually declare (`GPL-3.0`, `AGPL-3.0`, `LGPL-3.0`) reported as *unclassified* rather than *forbidden* | The same run | Not as a security hole — unclassified fails closed, so the verdict was right. As a wrong diagnosis on a correct verdict: "nobody has read this licence" when the truth is "remove this package" |
| **The CI pipeline could never have run.** It referenced `Edpf.sln` in four places across two workflows; the solution is `Edpf.slnx` and has been since Phase 01, so every `build-and-test` job would have died at `dotnet restore`. The repository has no git remote, so the pipeline had never executed and nothing reported it | Checking whether CI actually ran the suites just found to be skipped — the README claims the public API is "diffed in CI" | **As every commit gate in Appendix Z.13 being decorative.** Warnings-as-errors, the architecture tests, the isolation suite, the API baseline diff, the secret scan — all configured, none enforced |
| A **second** CI defect behind the first: the SBOM step passed `--disable-github-licenses`, removed in CycloneDX 6.x, where GitHub licence resolution is off by default and opted *into* with `--enable-github-licenses`. It aborted with "Unrecognized command or argument" before reading a project | Installing CycloneDX and running the step locally rather than trusting the earlier filename fix | As a second red build immediately after the first was fixed. Two defects in one broken step is what happens when nothing has ever executed it — the first failure masks everything behind it |
...
| `edpf check-licenses` | Documented in the README as a build gate; only `edpf doctor` had ever been run | **Broken.** Evaluated the CSV header as a dependency, so it failed on every natural input |

Five of six were broken. The one that was fine is the reason to run them all
